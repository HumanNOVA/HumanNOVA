...
def forward_attn(
    ...
    mixed_query_layer = self.query(hidden_states)
    delta_q = self.lora_q(hidden_states)
    mixed_query_layer  = mixed_query_layer  + delta_q
    ...
    if hasattr(F, "scaled_dot_product_attention"):
        ...
        key_layer = self.key(hidden_states).reshape(new_size).transpose(1, 2)
        value_layer = (self.value(hidden_states) + self.lora_v(hidden_states)).reshape(new_size).transpose(1, 2)
        ...
        key_layer = self.transpose_for_scores(self.key(hidden_states))
        value_layer = self.transpose_for_scores(self.value(hidden_states) + self.lora_v(hidden_states))
        query_layer = self.transpose_for_scores(mixed_query_layer)
...
class LoRA(nn.Module):
    def __init__(self, in_dim, out_dim, r=8):
        ...
        self.adapter_down = nn.Linear(in_dim, r, bias=False)
        self.adapter_up = nn.Linear(r, out_dim, bias=False)
...
def set_adapter_dino(model, in_dim=32, out_dim=32, r=8):
    for layer in model.children():
        if type(layer) == Dinov2SelfAttention:
            layer.lora_q = LoRA(in_dim, out_dim, r)
            layer.lora_v = LoRA(in_dim, out_dim, r)
            bound_method = forward_attn.__get__(layer, layer.__class__)
            setattr(layer, 'forward', bound_method)
        elif len(list(layer.children())) != 0:
            set_adapter_dino(layer, in_dim, out_dim, r)