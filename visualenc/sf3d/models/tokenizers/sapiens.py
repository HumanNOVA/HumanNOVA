...
class VisionTransformer(BaseBackbone):
    ...
    def forward(self, x, modulation_cond = None):
        B = x.shape[0]
...
def forward_attn(self, x):
    ...
    delta_q = self.lora_q(x).reshape(B, N, 1, self.num_heads, self.head_dims).permute(2, 0, 3, 1, 4)
    delta_v = self.lora_v(x).reshape(B, N, 1, self.num_heads, self.head_dims).permute(2, 0, 3, 1, 4)
...
def set_adapter(model, in_dim=32, out_dim=32, r=8):
    for layer in model.children():
        if type(layer) == MultiheadAttention:
            ...
            set_adapter(layer, in_dim, out_dim, r)
