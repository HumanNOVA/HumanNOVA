    ...
    def _forward_(
        ...
        **kwargs,
        ...
        rays = Rays(
            directions=rays_d,
            origins=rays_o / 0.87,
            grid_idx=torch.zeros(num_rays, dtype=torch.int32).cuda(),
            near=t_near.squeeze() / 0.87,
            far=t_far.squeeze() / 0.87,
        )
        # ori xy xz yz
        triplane_new = rearrange(triplane, "Np Cp Hp Wp -> Np Hp Wp Cp")
        xy_fplane = torch.einsum('jkl,lm->jkm', [triplane_new[0], self.first_weight[0:40]]).float() # HW C
        xz_fplane = torch.einsum('jkl,lm->jkm', [triplane_new[1], self.first_weight[40:80]]).float()
        yz_fplane = torch.einsum('jkl,lm->jkm', [triplane_new[2], self.first_weight[80:120]]).float()
        # lightplane yx zx zy
        feature_grid = [
            rearrange(xy_fplane, "X Y Cp -> () () X Y Cp"),
            rearrange(xz_fplane, "X Z Cp -> () X () Z Cp"),
            rearrange(yz_fplane, "Y Z Cp -> () Y Z () Cp"),
        ]
        ...
        (
            ...
            rendered_alpha,
            rendered_features,
        ) = self.lightplane_renderer(rays=rays, feature_grid=feature_grid)
        ...
        comp_rgb_ = rendered_features
        opacity_ = rendered_alpha
